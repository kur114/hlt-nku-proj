...
from transformer.Layers import FFTBlock, FFCBlock, PreNet, PostNet, Linear
...
class Encoder(nn.Module):
    ...
    def __init__(self,
        ...
        if block_type == 'FFTBlock':
            self.layer_stack = nn.ModuleList([FFTBlock(
                d_model, d_inner, n_head, d_k, d_v, dropout=dropout) for _ in range(n_layers)])
        elif block_type == 'FFCBlock':
            self.layer_stack = nn.ModuleList([FFCBlock(
                d_model, d_inner, n_head, d_k, d_v, dropout=dropout) for _ in range(n_layers)])
        else:
            raise ValueError("block_type should be either 'FFTBlock' or 'FFCBlock'")
