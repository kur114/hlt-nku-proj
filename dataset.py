...
def reprocess_tensor(batch, cut_list):
    ...
    for length_src_row in length_text:
        # src_pos: [batch_size, max_len]， 1234567...0000
...
if __name__ == "__main__":
    ...
    # test if all mel_len <= 2048
    max_len = 0
    for i, data in enumerate(tqdm(dataset)):
        max_len = data['mel_target'].shape[0] if data['mel_target'].shape[0] > max_len else max_len
        if data['mel_target'].shape[0] > 2048:
            print(f"Error: len > 2048: sample text: {data['text']}")
            break
