...
def create_alignment(base_mat, duration_predictor_output):
    # base_mat: (batch_size, mel_max_len, phoneme_max_len) = 0/1
    # duration_predictor_output: (batch_size, phoneme_max_len) = times
    # goal: times -> max_mel_len
...
class LengthRegulator(nn.Module):
    ...
    def LR(self, x, duration_predictor_output, mel_max_length=None):
        ...
        # alignment: (batch_size, mel_max_len, phoneme_max_len)
        alignment = create_alignment(alignment,
                                     duration_predictor_output.cpu().numpy())
        alignment = torch.from_numpy(alignment).to(device)
        # output: (batch_size, mel_max_len, encoder_dim)
        output = alignment @ x
        if mel_max_length:
            output = F.pad(
    ...
    def forward(self, x, alpha=1.0, target=None, mel_max_length=None):
        ...
        if target is not None: # for training
...
class UNet1D(nn.Module):
    def __init__(self, num_mels, max_length=2048):
    ...
    def forward(self, x, mask):
        ...
        assert x.size(1) == self.num_mels

        if x.size(2) > self.max_length:
            raise ValueError(f"Input length {x.size(2)} exceeds max_length {self.max_length}")
        
        # Pad x and mask to max_length if necessary
        original_length = x.size(2)
        if original_length < self.max_length:
            pad_length = self.max_length - original_length
            x = F.pad(x, (0, pad_length), mode='constant', value=0)
            mask = F.pad(mask, (0, pad_length), mode='constant', value=0)

        # fake timestep & unet denoising
        timestep = torch.zeros(x.shape[0], dtype=torch.long, device=x.device)
        noise_pred = self.unet(x, timestep, return_dict=False)[0]
